...
export class Admin {
  ...
  static async authenticate(email, password) {
    try {
      ...
      // Ensure in-memory admins are initialized first
      await ensureInitialized()
      ...
      if (!admin) {
        ...
        console.log(`Available admins: ${Array.from(inMemoryAdmins.keys()).join(', ')}`)
        return null
      }

      console.log(`✅ Admin found: ${email}, checking password...`)

      // Handle both hashed and plain passwords for development
      let isValidPassword = false
      try {
        isValidPassword = await bcrypt.compare(password, admin.password)
      } catch (bcryptError) {
        console.log(`⚠️ bcrypt error, trying direct comparison: ${bcryptError.message}`)
        // Fallback for development - direct comparison
